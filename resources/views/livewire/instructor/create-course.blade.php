<div class="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    @if (session('createCourse'))
        <x-popup message="{{ session('createCourse') }}" bgColor="bg-green-500" darkBgColor="dark:bg-green-900" />
    @endif

    <div class="dark:bg-[#161615] space-y-2" x-cloak x-transition>
        <div class="flex flex-col">
            <label class="text-2xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">สร้างคอร์สเรียนใหม่</label>
            <label class="text-lg text-[#706f6c] dark:text-[#A1A09A]">กรอกข้อมูลคอร์สด้านล่าง</label>
        </div>
            ...
            @elseif ($step === 3)
                ...
                <div class="flex justify-between max-w-5xl mx-auto mt-6">
                    ...
                    <button type="button" wire:click="create"
                        class="cursor-pointer w-[250px] group/button relative inline-flex items-center justify-center overflow-hidden rounded-md bg-blue-500 backdrop-blur-lg px-4 sm:px-6 py-2 text-base font-semibold text-white transition-all duration-300 ease-in-out hover:shadow hover:shadow-blue-600/50 border border-white/20">
                        <span class="text-base sm:text-lg">สร้างคอร์ส</span>
                    </button>
                </div>
            @endif
        </div>
        ...
        <script>
            function initializePikaday() {
                ...
                if (startDateField && !startDateField.pikaday) {
                    ...
                    startPicker = new Pikaday({
                        ...
                        onSelect: function(date) {
                            ...
                            // อัปเดต minDate สำหรับ end_date และ training_date
                            if (endPicker) {
                                const currentEndDate = endPicker.getDate();
                                if (currentEndDate && currentEndDate < date) {
                                    console.log('Clearing end_date as it is before start_date');
                            ...
                            if (trainingPicker) {
                                const currentTrainingDate = trainingPicker.getDate();
                                if (currentTrainingDate) {
                                    console.log('Clearing training_date as start_date changed');
                                    trainingPicker.setDate(null);
                                    @this.set('training_date_formatted', '');
                                    @this.set('training_date', '');
                                }
                                // training_date minDate จะขึ้นอยู่กับ end_date ดังนั้นไม่ตั้ง minDate ตรงนี้
                            }
                            if (endTrainingPicker) {
                                const currentEndTrainingDate = endTrainingPicker.getDate();
                                if (currentEndTrainingDate) {
                                    console.log('Clearing end_training_date as start_date changed');
                                    endTrainingPicker.setDate(null);
                                    @this.set('end_training_date_formatted', '');
                                    @this.set('end_training_date', '');
                                }
                                // end_training_date minDate จะขึ้นอยู่กับ training_date ดังนั้นไม่ตั้ง minDate ตรงนี้
                            }
                        }
                    });
                ...
                if (endDateField && !endDateField.pikaday) {
                    ...
                    endPicker = new Pikaday({
                        ...
                        onSelect: function(date) {
                            ...
                            // อัปเดต minDate สำหรับ training_date
                            if (trainingPicker) {
                                const currentTrainingDate = trainingPicker.getDate();
                                const nextDay = new Date(date);
                                nextDay.setDate(date.getDate() + 1); // วันที่ถัดจาก end_date
                                if (currentTrainingDate && currentTrainingDate < nextDay) {
                                    console.log('Clearing training_date as it is before next day of end_date');
                                    trainingPicker.setDate(null);
                                    @this.set('training_date_formatted', '');
                                    @this.set('training_date', '');
                                }
                            ...
                            if (endTrainingPicker) {
                                const currentEndTrainingDate = endTrainingPicker.getDate();
                                if (currentEndTrainingDate) {
                                    console.log('Clearing end_training_date as end_date changed');
                                    endTrainingPicker.setDate(null);
                                    @this.set('end_training_date_formatted', '');
                                    @this.set('end_training_date', '');
                                }
                ...
                if (trainingDateField && !trainingDateField.pikaday) {
                    ...
                    // คำนวณ minDate สำหรับ training_date จาก end_date
                    let trainingMinDate = parseDate(endDateField.value);
                    if (trainingMinDate) {
                        trainingMinDate = new Date(trainingMinDate);
                        trainingMinDate.setDate(trainingMinDate.getDate() + 1); // วันที่ถัดจาก end_date
                    } else {
                        trainingMinDate = new Date(); // ถ้าไม่มี end_date ใช้วันปัจจุบัน
                    }
                    ...
                    trainingPicker = new Pikaday({
                        ...
                        onSelect: function(date) {
                            ...
                            // อัปเดต minDate สำหรับ end_training_date
                            if (endTrainingPicker) {
                                const currentEndTrainingDate = endTrainingPicker.getDate();
                                if (currentEndTrainingDate && currentEndTrainingDate < date) {
                                    console.log('Clearing end_training_date as it is before training_date');
                                    endTrainingPicker.setDate(null);
                                    @this.set('end_training_date_formatted', '');
                                    @this.set('end_training_date', '');
                                }
                ...
                if (endTrainingDateField && !endTrainingDateField.pikaday) {
                    ...
                    // คำนวณ minDate สำหรับ end_training_date จาก training_date
                    let endTrainingMinDate = parseDate(trainingDateField.value);
                    if (endTrainingMinDate) {
                        endTrainingMinDate = new Date(endTrainingMinDate); // minDate เท่ากับ training_date
                    } else {
                        endTrainingMinDate = new Date(); // ถ้าไม่มี training_date ใช้วันปัจจุบัน
                    }
                ...
                if (window.Livewire) {
                    Livewire.on('updatePikaday', (data) => {
                        ...
                        if (data.start_date && endPicker) {
                            const minDate = new Date(data.start_date);
                            console.log('Setting end_date minDate:', minDate);
                            endPicker.setMinDate(minDate);
                            const currentEndDate = endPicker.getDate();
                            if (currentEndDate && currentEndDate < minDate) {
                                console.log('Clearing end_date due to minDate violation');
                                endPicker.setDate(null);
                                @this.set('end_date_formatted', '');
                                @this.set('end_date', '');
                            }
                        }
                        if (data.end_date && trainingPicker) {
                            const minDate = new Date(data.end_date);
                            minDate.setDate(minDate.getDate() + 1); // วันที่ถัดจาก end_date
                            console.log('Setting training_date minDate:', minDate);
                            trainingPicker.setMinDate(minDate);
                            const currentTrainingDate = trainingPicker.getDate();
                            if (currentTrainingDate && currentTrainingDate < minDate) {
                                console.log('Clearing training_date due to minDate violation');
                                trainingPicker.setDate(null);
                                @this.set('training_date_formatted', '');
                                @this.set('training_date', '');
                            }
                        }
                        if (data.training_date && endTrainingPicker) {
                            const minDate = new Date(data.training_date);
                            console.log('Setting end_training_date minDate:', minDate);
                            endTrainingPicker.setMinDate(minDate);
                            const currentEndTrainingDate = endTrainingPicker.getDate();
                            if (currentEndTrainingDate && currentEndTrainingDate < minDate) {
                                console.log('Clearing end_training_date due to minDate violation');
                                endTrainingPicker.setDate(null);
                                @this.set('end_training_date_formatted', '');
                                @this.set('end_training_date', '');
                            }
                        ...
                        if (startPicker && data.start_date_formatted) {
                            console.log('Updating start_date Pikaday:', data.start_date_formatted);
                        ...
                        if (endPicker && data.end_date_formatted) {
                            console.log('Updating end_date Pikaday:', data.end_date_formatted);
                        ...
                        if (trainingPicker && data.training_date_formatted) {
                            console.log('Updating training_date Pikaday:', data.training_date_formatted);
                        ...
                        if (endTrainingPicker && data.end_training_date_formatted) {
                            console.log('Updating end_training_date Pikaday:', data.end_training_date_formatted);
...
<script>
    document.title = "เพิ่มคอร์สเรียน";
</script>
